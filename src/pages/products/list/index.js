    ...
    render(){
        ...
        let productsCards = productModel.items.map((product) => {  
            ...
            if(cart.inCart(product.id)){
                btn = <Button variant="danger" onClick={() => cart.remove(product.id)} disabled={product.id in cart.processId}>
                    Remove from cart
                </Button>
            }
            else{
                btn = <Button variant="success" onClick={() => cart.add(product.id)} disabled={product.id in cart.processId}>
                    Add to cart
                </Button>
            }
